...
# ==========================
# GEMINI SENTENCE ANALYSIS
# ==========================
def analyze_with_gemini(sentence_words):
    """Generate natural Hindi sentence from ISL words using Gemini"""
    try:
        if not sentence_words:
            return "❌ No words to analyze. Please add words first."
...
Output the Hindi sentence:"""
        ...
        return f"🧠 Natural Hindi Sentence:\n\n{hindi_sentence}\n\n📝 Original words: {words_str}"
...
def reset_sentence():
    ...
    return [], "🔄 Sentence cleared. Ready for new words."

# ==========================
# UI
# ==========================
def launch():
    with gr.Blocks(title="ISL Sentence Builder") as demo:
        gr.Markdown(
            "# 🤟 ISL Sentence Builder\n"
            "**Record or upload sign videos → Build sentence → Generate natural Hindi**"
        )
        ...
        with gr.Row():
            with gr.Column():
                # Webcam Section
                gr.Markdown("### 🎥 Record One Sign")
                cam = gr.Video(sources=["webcam"], format="mp4", label="Webcam")
                cam_out = gr.Textbox(lines=4, label="Prediction Result")
                gr.Button("➕ Add Recorded Word", variant="primary").click(
                    predict_word,
                    inputs=[cam, sentence_state],
                    outputs=[sentence_state, cam_out]
                )

            with gr.Column():
                # Upload Section
                gr.Markdown("### 📁 Upload Sign Videos")
                files = gr.File(
                    file_types=["video"], 
                    file_count="multiple",
                    label="Upload Videos"
                )
                upload_out = gr.Textbox(lines=4, label="Upload Results")
                gr.Button("➕ Add Uploaded Words", variant="primary").click(
                    predict_multiple_videos,
                    inputs=[files, sentence_state],
                    outputs=[sentence_state, upload_out]
                )

        gr.Markdown("---")

        # Sentence Analysis Section
        gr.Markdown("### 🧠 Generate Hindi Sentence")
        with gr.Row():
            analyze_btn = gr.Button("🧠 Generate Hindi Sentence (Gemini)", variant="secondary", size="lg")
            reset_btn = gr.Button("🔄 Clear Sentence", variant="stop")
        
        analyze_out = gr.Textbox(lines=4, label="Hindi Sentence")
        ...
        gr.Markdown(
            "---\n"
            "💡 **Tips:** Record clear signs, ensure good lighting, and keep hands visible in frame."
        )
