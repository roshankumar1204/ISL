...
def launch():
    with gr.Blocks(title="ISL Sentence Builder") as demo:
        gr.Markdown("# 🤟 ISL Sentence Builder")
        gr.Markdown("Record signs → Build sentence → Get Hindi translation")

        sentence_state = gr.State([])

        # Single column layout
        gr.Markdown("### Record or Upload Signs")
        
        with gr.Tabs():
            with gr.Tab("📹 Webcam"):
                cam = gr.Video(sources=["webcam"], format="mp4")
                gr.Button("Add Sign", variant="primary", size="lg").click(
                    predict_word,
                    inputs=[cam, sentence_state],
                    outputs=[sentence_state, cam_out]
                )
                cam_out = gr.Textbox(label="Result", lines=3)

            with gr.Tab("📁 Upload"):
                files = gr.File(file_types=["video"], file_count="multiple")
                gr.Button("Add Signs", variant="primary", size="lg").click(
                    predict_multiple_videos,
                    inputs=[files, sentence_state],
                    outputs=[sentence_state, upload_out]
                )
                upload_out = gr.Textbox(label="Result", lines=3)

        gr.Markdown("---")
        
        # Action buttons
        with gr.Row():
            gr.Button("🧠 Generate Hindi", variant="secondary", size="lg").click(
                analyze_with_gemini,
                inputs=[sentence_state],
                outputs=[analyze_out]
            )
            gr.Button("🔄 Clear", size="lg").click(
                reset_sentence,
                outputs=[sentence_state, cam_out, upload_out]
            )
